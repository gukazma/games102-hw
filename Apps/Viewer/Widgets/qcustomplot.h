...
enum SelectionRectMode
{
    ...
    ,
    srmZoom     ///< When dragging the mouse, a selection rect becomes active. Upon releasing, the
                ///< axes that are currently set as range zoom axes (\ref
                ///< QCPAxisRect::setRangeZoomAxes) will have their ranges zoomed accordingly.
    ,
    ...
    srmCustom   ///< When dragging the mouse, a selection rect becomes active. It is the
                ///< programmer's responsibility to connect according slots to the selection rect's
                ///< signals (e.g. \ref QCPSelectionRect::accepted) in order to process the user
                ///< interaction.
};
...
enum SelectionType
{
    ...
    ,
    stWhole        ///< Selection behaves like \ref stMultipleDataRanges, but if there are any data
                   ///< points selected, the entire plottable is drawn as selected.
    ,
...
public:
    ...
    */
    enum SizeConstraintRect
    {
        ...
        ,
        scrOuterRect   ///< Minimum/Maximum size constraints apply to outer rect, thus include
                       ///< layout element margins
    };
    ...
    */
    enum ScaleStrategy
    {
        ssNone   ///< Modifications are not allowed, the specified tick step is absolutely fixed.
                 ...
                 ///< is zoomed out.
        ,
        ssMultiples   ///< An integer multiple of the specified tick step is allowed. The used
                      ///< factor follows the base class properties of \ref setTickStepStrategy and
                      ///< \ref setTickCount.
        ,
...
template<class DataType> void QCPDataContainer<DataType>::removeBefore(double sortKey)
{
    ...
    mPreallocSize +=
        int(itEnd - it);   // don't actually delete, just add it to the preallocated block (if it
...
template<class DataType> void QCPDataContainer<DataType>::remove(double sortKey)
{
    ...
    if (it != end() && it->sortKey() == sortKey) {
        if (it == begin())
            ++mPreallocSize;   // don't actually delete, just add it to the preallocated block (if
...
public:
    ...
    */
    enum PositionType
    {
        ...
        ,
        ptAxisRectRatio   ///< Static positioning given by a fraction of the axis rect size (see
                          ///< \ref setAxisRect). For example, if you call setCoords(0, 0), the
                          ///< position will be at the top left corner of the axis rect.
                          ///< setCoords(1, 1) will be at the bottom right corner, setCoords(0.5, 0)
                          ///< will be horizontally centered and vertically at the top of the axis
                          ///< rect, etc. You can also go beyond the axis rect by providing negative
    ...
    */
    enum RefreshPriority
    {
        ...
        rpQueuedRefresh      ///< Replots immediately, but queues the widget repaint, by calling
                             ///< QWidget::update() after the replot. This way multiple redundant
                             ///< widget repaints can be avoided.
        ,
                                                                  ...
                                                                  bool onlySelectable) const
{
    ...
    QCPRange keyRange(key1, key2);   // QCPRange normalizes internally so we don't have to care
                                     // about whether key1 < key2
...
public:
    ...
    */
    enum BracketStyle
    {
        ...
        ,
        bsHalfEllipse     ///< A half ellipse is drawn. The size of the ellipse is given by the
...
public:
    ...
    */
    enum SpacingType
    {
        ...
        ,
        stPlotCoords      ///< Bar spacing is in key coordinates and thus scales with the key axis
                          ///< range
    };
    Q_ENUMS(SpacingType)
...
public:
    ...
    */
    enum ErrorType
    {
        etKeyError     ///< The errors are for the key dimension (bars appear parallel to the key
                       ///< axis)
        ,
        etValueError   ///< The errors are for the value dimension (bars appear parallel to the
                       ///< value axis)
    };
    Q_ENUMS(ErrorType)
